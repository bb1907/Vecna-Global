          ...
          <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 100 }} className="fixed inset-x-4 bottom-4 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 z-[101] w-full max-w-xl bg-[#0f172a] border border-white/10 shadow-2xl p-8 md:p-12 rounded-[32px] text-white">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <input required placeholder="Full Name" className="w-full bg-transparent border-b border-white/10 py-3 focus:outline-none focus:border-primary-yellow transition-colors" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                <input required type="email" placeholder="Email Address" className="w-full bg-transparent border-b border-white/10 py-3 focus:outline-none focus:border-primary-yellow transition-colors" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
              </div>
              <input required placeholder="Inquiry Subject" className="w-full bg-transparent border-b border-white/10 py-3 focus:outline-none focus:border-primary-yellow transition-colors" value={formData.subject} onChange={e => setFormData({ ...formData, subject: e.target.value })} />
              <textarea required rows={4} placeholder="Briefly state your orchestrational goal" className="w-full bg-transparent border-b border-white/10 py-3 focus:outline-none focus:border-primary-yellow transition-colors resize-none" value={formData.message} onChange={e => setFormData({ ...formData, message: e.target.value })} />
              <button className="w-full bg-primary-yellow text-black py-5 rounded-full font-bold flex items-center justify-center gap-3 hover:bg-primary-yellow/80 transition-all active:scale-[0.98]">
    ...
    <div className="min-h-screen bg-[#020617] selection:bg-primary-yellow selection:text-black overflow-x-hidden text-white font-sans">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-[#020617]/80 backdrop-blur-md border-b border-white/5 py-4" : "bg-transparent py-8"}`}>
        <div className="max-w-[1200px] mx-auto px-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <button onClick={() => setIsContactModalOpen(true)} className="bg-white text-black px-6 py-2.5 text-sm font-bold rounded-full hover:bg-primary-yellow transition-all">Request Briefing</button>
          </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, x: "100%" }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: "100%" }} className="fixed inset-0 z-40 bg-[#020617] pt-32 px-10 md:hidden flex flex-col gap-10">
            ...
            <button onClick={() => { setIsMobileMenuOpen(false); setIsContactModalOpen(true); }} className="w-full bg-primary-yellow text-black py-6 text-xl font-bold rounded-2xl">Request Briefing</button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <main className="relative">
        ...
        <div className="absolute top-0 left-0 right-0 h-[1000px] pointer-events-none z-0 overflow-hidden">
          <div className="absolute top-[-200px] left-1/2 -translate-x-1/2 w-[1000px] h-[1000px] bg-primary-yellow/10 blur-[150px] rounded-full" />
        ...
        <div className="relative z-10 max-w-[1200px] mx-auto px-6">
          ...
          <section id="home" className="pt-40 pb-32 md:pt-60 md:pb-52 text-center flex flex-col items-center">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-col sm:flex-row gap-6">
              <button onClick={() => setIsContactModalOpen(true)} className="bg-primary-yellow text-black px-10 py-5 rounded-full font-bold text-lg hover:bg-primary-yellow/80 transition-all flex items-center gap-3">
          ...
          <section id="what-we-do" className="py-32 scroll-mt-32">
            <div className="mb-20 text-center">
              <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-primary-yellow mb-4 italic">Operational Foundation</h2>
          ...
          <section id="architecture" className="py-32 scroll-mt-32 border-y border-white/5">
            <div className="mb-20 text-center">
              <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-primary-yellow mb-4">Structural Blueprint</h2>
          ...
          <section id="brands" className="py-32 scroll-mt-32">
            <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-20">
              <div className="max-w-2xl">
                <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-primary-yellow mb-4">Ecosystem Layers</h2>
          ...
          <section className="py-32 scroll-mt-32 border-t border-white/5">
            <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-primary-yellow/50 mb-12">Product Capability Matrix</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {VENTURE_CAPS.map((cap, i) => (
                <div key={i} className="flex flex-col gap-6">
                  <div className="size-14 bg-primary-yellow/10 rounded-2xl flex items-center justify-center text-primary-yellow">
                    <cap.icon size={28} />
                  </div>
          ...
          <section className="py-32 scroll-mt-32">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
              <div>
                <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-primary-yellow mb-4">Economic Backbone</h2>
                ...
                <div className="space-y-4">
                  {["Central Treasury", "Controlled Payout", "Venture-Based Entitlement"].map(item => (
                    <div key={item} className="flex items-center gap-4 p-4 bg-white/5 border border-white/10 rounded-2xl">
                      <div className="size-3 bg-primary-yellow rounded-full" />
          ...
          <section id="team" className="py-32 scroll-mt-32 border-t border-white/5">
            <div className="mb-20">
              <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-primary-yellow mb-4">Founder & Execution Board</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {FOUNDERS.map((founder, i) => (
                ...
                >
                  <div className="size-12 bg-primary-yellow/10 rounded-xl flex items-center justify-center text-primary-yellow mb-6 group-hover:bg-primary-yellow group-hover:text-black transition-all">
                    <founder.icon size={24} />
                  </div>
                  <h3 className="text-xl font-bold mb-1 tracking-tight">{founder.name}</h3>
                  <div className="text-primary-yellow text-[10px] font-black uppercase tracking-[0.2em] mb-4">{founder.role}</div>
          ...
          <section className="py-32 scroll-mt-32 border-t border-white/5">
            <div className="flex flex-col md:flex-row gap-20">
              <div className="flex-1">
                ...
                <div className="space-y-2">
                  {["Founder Core", "Execution Companies", "Founding Builders", "Active Builder Network", "Partners & Stakeholders", "Audience Layer"].map((tier, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-white/5 border-l-2 border-primary-yellow/20 hover:border-primary-yellow transition-all">
                        <span className="font-bold text-sm">{tier}</span>
                        <div className="size-1.5 rounded-full bg-primary-yellow/30" />
                     </div>
                  ))}
                </div>
              </div>
              <div className="flex-1 bg-gradient-to-br from-white/5 to-transparent p-12 rounded-[40px] border border-white/10 flex flex-col justify-center items-center text-center">
                 <Globe className="text-primary-yellow size-16 mb-8 opacity-50" />
          ...
          {/* SECTION 10 — Roadmap */}
          <section className="py-32 scroll-mt-32 border-t border-white/5">
            <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-primary-yellow mb-12">First 30 Days Milestone</h2>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {ROADMAP.map((item, i) => (
                <div key={i} className="relative p-6 bg-white/5 border border-white/10 rounded-2xl group overflow-hidden">
                  <div className="text-[40px] font-black text-white/5 absolute top-2 right-2 group-hover:text-primary-yellow/10 transition-colors leading-none">{item.step}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {INVESTMENT_WHY.map((item, i) => (
                <div key={i} className="p-10 border border-white/5 bg-[#0f172a] rounded-[40px]">
                  <h3 className="text-2xl font-bold mb-6 tracking-tight text-primary-yellow">{item.title}</h3>
      ...
      <footer className="border-t border-white/5 py-12 bg-black/50 backdrop-blur-3xl">
        <div className="max-w-[1200px] mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex flex-wrap justify-center gap-8">
            {NAV_LINKS.map(item => (
              <a key={item.id} href={`#${item.id}`} className="text-[10px] font-black text-white/30 hover:text-primary-yellow transition-colors uppercase tracking-[0.3em] italic">{item.name}</a>
            ))}
          </div>
